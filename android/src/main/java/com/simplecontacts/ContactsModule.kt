...
class ContactsModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(), PermissionListener {
    ...
    private var getContactsPromise: Promise? = null
    ...
    fun getContacts(promise: Promise) {
        if (hasPermission()) {
            ...
            // Store the promise for later use after permission is granted
            getContactsPromise = promise
            ...
            if (activity != null && activity is PermissionAwareActivity) {
                ...
                promise.reject("permission_denied", "Contacts permission not granted and cannot request it")
            }
        }
    }
    
    private fun fetchContacts(promise: Promise) {
        try {
